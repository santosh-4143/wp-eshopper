...
function wccm_render_catalog_compare_info() {
	...
	echo '<div class="wccm-catalog-items">';
       ...
       echo '<div class="col-lg-10 com_list">';
       echo '<ul>';
		...
		foreach ( $list as $product_id ) {
			...
			if ( $product ) {
				echo '<div class="wccm-catalog-item">';
					...
					echo '<a class="dashicons dashicons-no re_move" href="', esc_url( wccm_get_compare_link( $product_id, 'remove-from-list' ) ), '"><i class="fa fa-2x fa-remove r_1"></i></a>';
				echo '</div>';
			}
                               echo '</li>';
		}
		...
		$compare_link = wccm_get_compare_page_link( $list );
                echo '<div class="col-lg-2 c_list">';
		echo '<div class="wccm-catalog-item">';
                
			echo '<a class="button c_1" href="', esc_url( $compare_link ), '">', esc_html__( 'Compare', 'wccm' ), '</a>';
		echo '</div>';

		echo '<div class="wccm-catalog-item">';
			echo '<a class="button alt c_2" href="', esc_url( wccm_get_compare_link( implode( ',', $list ), 'remove-from-list' ) ), '">', esc_html__( 'Cancel', 'wccm' ), '</a>';
		echo '</div>';
                echo '</div>';
